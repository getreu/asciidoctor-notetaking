...
IMPORTANT: This project is discontinued. Please use 
http://blog.getreu.net/projects/pandoc-notetaking/[pandoc-notetaking]
instead. The code of `pandoc-notetakting` is generic, works with most markup 
languages and can optionally use http://pandoc.org[pandoc] as renderer. 
...
* _a sequence number sort tag_.
+
.Sequence number sort tags
====
	02-
	08-
	09_02-
...
_chronological sort tag_ of today. The `<simplified-title>` part is derived
from the parent directory name omitting its own _sort tag_.

.New note in a directory with sequence number sort tag
====
...
NOTE: The parent directory's sort tag is never used to compose a filename for
a new note.
====

When `adoc-note` creates a new note based on a filename no
_sort tag_ is prepended.
...
the filename is changed on disk. Possible _sort tags_ remain untouched.
