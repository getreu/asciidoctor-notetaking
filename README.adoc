...
Markup languages like `Asciidoc` are perfectly suited for quick note taking.
...
The `adoc-note` script can operate in three modes depending on it's paramters: 

Create a new note::
	Creates new context depending asciidoc note files.

Edit notes:: 
	Launches an editor and live-previewer (Firefox, Chrome of Chromium).

View notes:: Launches a live-previewer.

The project is hosted on Github: 
https://github.com/getreu/asciidoctor-notetaking[getreu/asciidoctor-notetaking]

The program runs under Linux and Windows.footnote:[For Mac follow Linux
...
* Right click on a file or directory in your file-manger and choose _adoc-note_
  in the context menu. See <<_integration_with_file_browser>>. 
...
With reference to file: link:++<filename>++[]
----
====
...
[source, bash]
 adoc-new-note 'doc/Implementing the NIST Cybersecurity Framework.pdf' 
...
With reference to file: link:++Implementing-the-NIST-Cybersecurity-Framework.pdf++[]
----
====
...
All text in the above example was automatically generated by the `adoc-new-note`
...
. Copy them in a location of your `$PATH` and make them executable for everyone.
+
[sourace,bash]
...
+
[sourace,bash]
  sudo apt-get install dirname basename readlink sed uni2ascii vim-gtk
+
...
commments `CONFIGURATION SECTION START` and `CONFIGURATION SECTION END`.  Here
you can specify what editor you want to use, if you choose `firefox`, `chrome`
or `chromium` and you can specify the parameters these programs require.  Make
sure that your editor does not fork when launched. If it does fork, the script
will still work but when you quit, no asciidoc-title-filename sync will
occur.  Outside the `CONFIGURATION SECTION` no changes should be necessary.

. Optional: integrate the scripts with your file-manager (see below).
...
. Edit `adoc-note.bat` and adjust the `BIN_DIR` variable to point to your
_BIN_DIR_ directory. 
+
If you are working in a restricted environment where you do not have
installation rights you might want to use portable editor and viewer programs. 
In this case you can specify `VIEWER_DIR` and `EDITOR_DIR` in
`adoc-note.bak`. They will extend your `PATH` variable.
...
At the beginning of `adoc-note` you will find a section enclosed in the
commments `CONFIGURATION SECTION START` and `CONFIGURATION SECTION END`.  Here
you can specify what editor you want to use, e.g. if you choose `firefox.exe` or
`chromium.exe` and you can specify the parameters these programs require.  Do
the same for the editor of your choice. Make sure that your editor does not fork
when launched. If it does fork, the script will still work but when you quit, no
asciidoc-title-filename sync will occur.  Outside the `CONFIGURATION SECTION` no
changes should be necessary.

. Create a shortcut to `adoc-note.bat` on your desktop, click on properties 
and change the shortcut to _run minimize_.

. Launch `adoc-note` by dragging a file or directory on the shortcut.

=== Integration with file browser

`adoc-note` integrates easily with your favorite file browser. 
...
. Browse to the location of `adoc-bak` and click _Open_ and _Ok_.
+
image::explorer3.png[scaledwidth=70%]

.More information
* http://windows.microsoft.com/en-us/windows/change-file-open-program#1TC=windows-7[Change the program that opens a type of file]
* http://www.howtogeek.com/107965/how-to-add-any-application-shortcut-to-windows-explorers-context-menu/[How to Add Any Application Shortcut to Windows Explorer’s Context Menu].
